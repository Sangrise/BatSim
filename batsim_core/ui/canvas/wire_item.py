...
class WireItem(QGraphicsItem):
    ...
    def _blockers(self) -> list[QRectF]:
        ...
        def endpoint_blocker(it: ComponentItem, pin: int) -> QRectF:
            r = it.sceneBoundingRect().adjusted(3, 3, -3, -3)
            p = it.pin_scene_pos(pin)
            d = it.pin_exit_direction(pin)
            if d is None:
                return r
            # Do not block the legal pin-exit corridor.  The remaining
            # rect still represents the component body, so longer detours
            # cannot pass back through the symbol.
        ...
        for it in scene.items():
            if not isinstance(it, ComponentItem):
                continue
            if it is self.a_comp:
                # Endpoints: include their bbox slightly shrunk so the
                # approach stub stays outside, but a long detour segment
                # crossing the body interior is still rejected.
                out.append(endpoint_blocker(it, self.a_pin))
                continue
            if it is self.b_comp:
                out.append(endpoint_blocker(it, self.b_pin))
                continue

...
from batsim_core.ui.canvas.component_item import ComponentItem
from batsim_core.ui.canvas.wire_item import WireItem
from batsim_core.ui.palette import PaletteWidget
from batsim_core.ui.inspector import InspectorWidget
from batsim_core.ui.outline import OutlineWidget
from batsim_core.ui.waveform_view import WaveformView
from batsim_core.ui.simulation_dialog import SimulationDialog
...
from batsim_core.plugins import refresh_if_changed
from batsim_core.io.project import save_project, load_project
from batsim_core.io.blocks import (BLOCKS_DIR, list_blocks, load_block, save_block,
                              safe_block_name)
...
class _TransientWorker(QObject):
    ...
    def run(self) -> None:
        ...
        from batsim_core.engine.nonlinear import solve_transient as _st
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        d1 = QDockWidget("Component Palette", self)
        d1.setWidget(self.palette)
        d1.setMinimumWidth(300)
    ...
    def reload_cell_library(self):
        from batsim_core.plugins import discover, list_cells
        n = discover()
    ...
    def _paste_subgraph(self, sub: dict, anchor: QPointF | None = None) -> None:
        ...
        if anchor is not None:
            ...
            ox, oy = 20.0, 20.0
        # Re-id
        from batsim_core.ui.canvas.scene import CATALOG
        from batsim_core.ui.canvas.wire_item import WireItem
    ...
    def save_selection_as_block(self):
        ...
        if not ok or not name.strip():
            return
        try:
            safe = safe_block_name(name)
        except ValueError:
            return
        path = save_block(safe, sub)
        self._refresh_blocks_menu()
    ...
    def _refresh_blocks_menu(self):
        ...
        for name in names:
            ...
            act.triggered.connect(lambda _=False, n=name: self._insert_block(n))
            self._blocks_menu.addAction(act)

    def _insert_block(self, name: str):
        try:
            sub = load_block(name)
        except Exception as exc:
    ...
    def open_blocks_folder(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Blocks folder", str(BLOCKS_DIR))

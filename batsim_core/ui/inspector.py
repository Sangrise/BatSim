...
from batsim_core.plugins.registry import (BATTERY_MODELS, list_battery_models,
                                     list_cells)
...
class InspectorWidget(QWidget):
    ...
    def set_component(self, comp):
        ...
        for key, val in comp.params.items():
            ...
            if key == "model" and comp.kind in ("BATTERY", "BATPACK", "BATRACK"):
                ...
                from batsim_core.bms.cell_balancer import list_balancers
                box = QComboBox()
                ...
                def _open_editor(_=None, e=preview):
                    from batsim_core.ui.pcs_cycle_dialog import CycleStepsDialog
                    ...
                    if dlg.exec():
                        old_params = dict(self._comp.params)
                        ...
                        e.setText(new_json)
                        self._emit_graph_changed_if_needed(old_params)
                        self.set_component(self._comp)
    ...
    def _apply_cell_preset(self, cell_name: str):
        if self._comp is None or not cell_name:
            return
        old = dict(self._comp.params)
        from batsim_core.plugins.registry import CELL_LIBRARY
        ...
        self._comp.update()
        self._emit_graph_changed_if_needed(old)
    ...
    def _update_param(self, key, value):
        ...
        if isinstance(old, bool):
            ...
            try:
                self._comp.params[key] = int(value)
        ...
        if self._comp.params.get(key) == old:
            return
        self._comp.update()
        self._emit_graph_changed()

    def _emit_graph_changed_if_needed(self, old_params: dict) -> None:
        if self._comp is not None and dict(self._comp.params) != old_params:
            self._emit_graph_changed()

    def _emit_graph_changed(self) -> None:
        if self._comp is None:
            return
        scene = self._comp.scene()
